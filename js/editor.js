...
ZenPen.editor = (function () {
  ...
  function init() {
    ...
    const header_url =
      "https://database.deta.sh/v1/" + project_id + "/zenpad" + "/query";
    const query = {};
    ...
    loadState(header_url, requestOptions);
  ...
  function saveState(event) {
    ...
    const project_id = getProjectId(project_key);
    const header_url =
      "https://database.deta.sh/v1/" + project_id + "/zenpad" + "/items";
    const item = {
      items: [
        {
          key: "header",
          value: headerField.innerHTML,
        },
        {
          key: "body",
          value: contentField.innerHTML,
        },
      ],
    };
    const requestOptions = {
    ...
    fetch(header_url, requestOptions)
  ...
  function loadState(header_url, requestOptions) {
    fetch(header_url, requestOptions)
      .then(function (response) {
        ...
        // Examine the text in the response
        response.json().then(function (data) {
          let header = data["items"].filter(function (e) {
            return e.key === "header";
          });

          let body = data["items"].filter(function (e) {
            return e.key === "body";
          });
          if (!(header.length === 0) && header[0]["value"]) {
            headerField.innerHTML = header[0]["value"];
          } else {
            console.log(header["value"]);
            headerField.innerHTML = defaultTitle; // in default.js
          }
          if (!(body.length === 0) && body[0]["value"]) {
            contentField.innerHTML = body[0]["value"];
          } else {
            loadDefaultContent();
          }
        });
